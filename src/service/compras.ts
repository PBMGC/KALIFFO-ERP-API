...
export const _createCompra = async (compra: any) => {
    ...
    const result = await query(queryText, [empresa_proveedor,fecha_compra,cantidad,total,tienda_id]);
    
    console.log(empresa_proveedor,fecha_compra,cantidad,total,
        tienda_id,detalle)
...
export const _createCompraDetalle = async (compra_id:number,detalleCompra: any) => {
    ...
    for(const c of detalleCompra){
      ...
      const result = await query(queryText, [compra_id, c.producto, c.cantidad, c.total]);
    }
